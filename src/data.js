// Architect Editor — Fixture data (ES module)
...
const F = (...keys) => {
  let b = 0;
  for (const k of keys) {
    const def = ITEM_FLAGS.find(f => f.key === k);
    if (def) b |= def.bit;
  }
  return b;
};
...
const allItems = Object.values(DATA_ITEMS).flat().map(it => ({
  ...it,
  id: it.guid,
  name: it.displayName,
}));
...
export const DATA = {
  ...
  allItems,
  itemById:   Object.fromEntries(allItems.map(i => [i.guid, i])),
  itemByName: Object.fromEntries(allItems.map(i => [i.displayName, i])),
};
